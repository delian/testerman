...
##
# Slighty modified by Sebastien Lefevre:
# - supports basic implicit/explicit tagging
#   on command line (waiting for a correct implementation that uses "IMPLICIT TAGS" 
#   statement in ASN.1 definitions)
# - substitutes '_' to '-' in all Python identifiers (compiler.py now
#   only generates unmodified name strings)
# - updates ENUM syntax to preserve ASN.1 definition naming (using a dict
...
#		self.output ("#%s depends on %s" % (ident, str (dependencies))
			...
			for (ident, val) in self.assignments.iteritems ():
				...
				for d in self.dependencies [ident]:
					if (not already_output.has_key (d) and
						...
						ok = 0
				if ok:
					text_list.append ("%s=%s" % (ident.replace('-', '_'),
			...
			if not any_output:
				...
				for ident in self.assignments.iterkeys ():
					if not already_output.has_key (ident):
						...
						cycle_list.append ("%s(%s)" % (ident.replace('-', '_'), ",".join (depend_list)))
						
				text_list.append ("# Cycle XXX " + ",".join (cycle_list))
				promises_dict = {}
				for ident in cycle_ident_list:
					text_list.append("%s=%s" % (ident.replace('-', '_'),self.assignments[ident]))

				text_list.append ("_promises_dict = {")
				for ident in cycle_ident_list:
					text_list.append ("'%s' : %s," % (ident, ident.replace('-', '_')))
				text_list.append ("}")
				for ident in cycle_ident_list:
	...
	def visitType_Ref (self, node):
		self.output (node.name.replace('-', '_'))

	def visitSet_Of(self, node):
		s = self.visit_saving (node.val)
		self.output ('%sasn1.SET_OF (%s)' % (self.spaces (), s)) # XXX handle node.size_constr

	def visitSequence_Of(self, node):
		s = self.visit_saving (node.val)
		self.output ('%sasn1.SEQUENCE_OF (%s)' % (self.spaces (), s.replace('-', '_'))) # XXX handle node.size_constr
